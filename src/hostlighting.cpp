...
void HostLighting::registerLights(const Lights & lights) {
	...
	for (const Light & light : lights.AllLights) {
		...
		// Which slot this light draws its colour from in a profile's non-button
		// palette. Opaque to a host and deliberately not unique - several case
		// lights can share one - but it is the only grouping the board has, so
		// the light table carries it rather than dropping it here.
...
// Clamped against the staging buffer rather than trusting the caller: the case
// range collapses every case light into one {min first, summed count} span, so
// a user-edited light table with overlapping entries can describe more pixels
// than exist. Bounding the write here keeps a bad layout a lighting bug instead
// of a memory-corruption bug, for every present and future caller.
...
// Colours every light belonging to a control, not just the first. Boards may
// wire more than one physical button to the same action (the Haute42 B16 has a
// second Up and a second L3), and each of those buttons has its own light;
...
//
// Taken from the light registry rather than from thirty resolveButton calls.
// That is not only cheaper - one pass instead of one scan per control - it is
// the only version that works: resolveButton returns the first light matching a
// control and stops, so a hash built from it is blind to every duplicate light
// and to every position, which is precisely what page 5 exists to report.
//
// The pin map is hashed alongside it because ownership is resolved from the
// active profile at query time. The registry stores a pin, not an action, so a
// profile switch changes what page 5 says a light belongs to while leaving
// every registry byte identical.
static uint32_t ledMapFingerprint() {
	...
	// Board shape page 2 carries and the registry does not
...
static void buildCapsLedMap(uint8_t * reply) {
	...
	// Report every light the board has, not the sum of the per-control ranges:
	// a board that wires two buttons to one action has lights that no single
	// control range covers, and a host sizing a frame buffer needs the truth.
	// Taken straight from the registry rather than floored at that sum: the
	// case range above collapses every case light into one {min first, summed
	// count} span, which overstates the strip when case entries overlap, and a
	// count a host cannot trust as a bound is worse than one that is merely
	// smaller than it expected.
...
static void buildCapsState(uint8_t * reply) {
	...
	// This pipeline persists -1 for "no profile selected" (the lights-off
	// state); report that as the 0xFF sentinel rather than as a cast accident
	reply[11] = (animationOptions.baseProfileIndex < 0)
		? 0xFF : (uint8_t)animationOptions.baseProfileIndex;

	// Both bits describe this board right now, not what the firmware could do:
	// the registry is populated on the render core during LED setup, so a host
	// enumerating early can legitimately see them clear and should re-read.
	uint32_t features = 0;
...
// control): [3] current animation profile   [4] number of profiles
// Reports how many animation profiles the board actually has, not how many it
// could hold: selecting a slot with no profile behind it renders nothing.
...
// A strict projection of page 5's columns, kept because it fits 19 entries per
// read against page 5's 4, so a host that only wants positions keeps a cheap path.
...
// Which control owns a light, and what its pin actually does. The reverse of
// stageButton: take the pin's live action from the active profile and find it
// in the canonical table. Duplicates need no special case - two lights on one
// action simply report the same ID, which is the whole point of this page.
//
// Read live rather than cached at registration, because the registry is rebuilt
// only from the web configurator while a profile switch remaps pins underneath
// it. Caching here would freeze ownership at boot and quietly lie afterwards.
static void resolveLight(const RegisteredLight & light, uint8_t & buttonId,
	...
	switch (light.type) {
		...
		case LightType::LightType_Turbo:
			// The turbo light's stored pin is an LED driver pin, not a GPIO the
			// pin map knows, so it is never reported; the action named here is
			// the turbo button, which is a different thing on the same board
	...
	// Controls page 2 has no slot for are named only here. Anything else keeps
	// the unnameable ID and is identified by its action instead, which is why
	// that field travels verbatim.
...
// The echo makes a paged walk self-identifying, which matters because the first
// LED index is not a unique key - boards exist with two lights starting at the
// same index - and the fingerprint lets a host notice the map changing
// underneath a walk that takes several reads.
static void buildCapsLights(uint8_t * reply, uint8_t startEntry) {
	reply[6] = HOST_LIGHTING_LIGHT_STRIDE;

	// Written before any early exit. This value is what certifies pages 2, 4
	// and 5 as one consistent view, so a page reporting no records still has to
	// say which map it is reporting nothing for. Leaving it to the tail would
	// ship a zero here while pages 1 and 2 carry the real value at the same
	// instant, which is exactly the incoherence the fingerprint exists to rule
	// out. It is meaningful before the registry is populated: only the registry
	// walk inside ledMapFingerprint is gated on lightsReady, and the pin map it
	// hashes is live from boot.
	...
	for (uint8_t i = startEntry;
		...
		// The legacy layout converter can leave a light with no owning pin, and
		// the packing it uses turns that into an all-ones type. Clamp rather
		// than pass a value no host could interpret. Only the upper bound is
		// worth testing: LightType is unsigned, so nothing below zero arrives.
		...
		// This pipeline reads a real per-light table and real coordinates, so
		// it asserts both
...
// page 5 ordinal. On this pipeline an ordinal is an index into the light
// registry, which is exactly the order page 5 reports. The reply carries the
// counts in [3..4] and a per-entry outcome mask in [5..6] (bit n set =
// entry n applied), so a skip names the stale entry instead of forcing a
// page 5 re-walk.
		...
		case HOST_LIGHTING_CMD_FILL: {
			...
			switch (buffer[2]) {
				...
				case HOST_LIGHTING_FILL_SCOPE_BUTTONS:
					// Every button light, not only the eighteen page 2 can name.
					// Walking the registry rather than the canonical IDs means a
					// light on an extended control is filled like any other,
					// instead of staying dark while the rest of the panel changes
					// and looking to the user like a fault.
					if (lightsReady) {
						for (uint8_t i = 0; i < registeredLightCount; i++) {
